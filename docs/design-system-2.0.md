...
> ⚠️ **이 문서는 `mvp-front/docs/design-system.md` 로 통합됐다. 정본이 아니다.**
> 색·타이포·간격·모서리·그림자·공용 부품 규격은 전부 정본을 본다. 여기 남은 값은 옛 기록이고, 정본과 다르면 정본이 맞다.
> 특히 §2 의 `--color-success` · `--color-warning` 은 **코드에 없는 이름**이다. 실제 이름은 `--system-success` · `--system-warning` 이다.
> 이 문서는 2.0 리디자인 당시의 결정 배경(톤 블렌드·화면별 적용 메모)을 남기기 위해 삭제하지 않고 보존한다.
